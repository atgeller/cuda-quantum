...
def generate_program(template, seed, rz_weight, block_length, n_qubits):
    program = ""
    with open(template, 'r') as file:
        ...
        for raw in file:
            ...
            if line.startswith("GEN-QALLOC:"):
                # Allow local overriding of # of qubits
                # TODO: would be nice to do for block and subcircuit length too
                match = re.search(r"GEN-QALLOC: nqubits=(\d+)", line)
                if match:
                    nQubits = int(match.group(1))
                else:
                    nQubits = random.choice(n_qubits)
                ...
                if len(working_qubits) > 0:
                    line = line.replace("GEN:<nqubits>",
                                        str(len(working_qubits)))
                    line = line.replace("GEN:<qubit>",
                                        random.choice(working_qubits))
                if len(qvectors) > 0:
                    line = line.replace("GEN:<qvector>",
                                        random.choice(qvectors))
...
argparser.add_argument('--n-qubits', type=str)
